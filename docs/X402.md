...
### Setup del Middleware (Implementacion Actual)
...
// Cliente del Facilitator de Crypto.com
...
export const CAPABILITY_PRICES = {
  'contract-scan': { price: '0.01', description: 'Contract security scan' },
  'wallet-approvals': { price: '0.02', description: 'Wallet approvals check' },
  'tx-simulate': { price: '0.03', description: 'Transaction simulation' },
};

// Middleware que verifica pagos x402
export async function x402Middleware(req, res, next) {
  const capability = req.params.capability;
  const priceInfo = CAPABILITY_PRICES[capability];
  ...
  if (!paymentHeader) {
    // Retornar 402 con instrucciones de pago
    return res.status(402).json({
      price: priceInfo.price,
      token: 'USDC',
      network: config.network.name,
      recipient: config.recipientAddress,
      facilitator: config.facilitator.baseUrl,
    });
  }

  // Verificar pago con el facilitator
  const isValid = await facilitatorClient.verifyPayment(paymentHeader);
  if (!isValid) {
    return res.status(402).json({ error: 'PAYMENT_INVALID' });
  }
...
Ver `.env.testnet.example` para configuracion completa:
```env
CHAIN_ID=338
CRONOS_RPC_URL=https://evm-t3.cronos.org
CRONOS_EXPLORER_API=https://api-testnet.cronoscan.com/api
FACILITATOR_API_KEY=<tu_api_key>
FACILITATOR_BASE_URL=https://facilitator-testnet.crypto.com
```
